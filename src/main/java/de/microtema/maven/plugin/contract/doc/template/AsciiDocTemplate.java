...
public class AsciiDocTemplate implements DocTemplate {

    @Override
    public boolean access(String templateType) {

        return templateType.contains(".adoc");
    }
    ...
    public void writeOutDocFile(String outputDirectory, ClassDescriptor classDescriptor) throws IOException {
        ...
        if (isCommonClass) {
            stringBuilder.append(className).append(" Base class").append(MojoUtil.lineSeparator(2));

            List<String> interfaceNames = classDescriptor.getInterfaceNames();

            if (!interfaceNames.isEmpty()) {
                stringBuilder.append("Implements: ").append(MojoUtil.lineSeparator(1));
            }
            ...
            stringBuilder.append(entityDescriptor.getDescription()).append(MojoUtil.lineSeparator(2));
            stringBuilder.append("Version: ").append(entityDescriptor.getVersion()).append(MojoUtil.lineSeparator(2));
        ...
        stringBuilder.append("[cols=\"e,e,e,e,e,2e\" options=\"header\"]").append(MojoUtil.lineSeparator(1));
        ...
        for (FieldDescriptor fieldDescriptor : entityDescriptor.getFields()) {
            ...
            stringBuilder.append("| ").append(index++)
                    ...
                    .append(MojoUtil.lineSeparator(1));
        }

        stringBuilder.append("|===").append(MojoUtil.lineSeparator(1));

        String file = String.format("%s%s%s.adoc", outputDirectory, File.separator, className);

        System.out.println("Writing Ascii Doc file " + file);
